...
def top_correlations(df, n=10):
    ...
    corr.columns = ['feature_1', 'feature_2', 'corr_val']
    corr = corr[corr['feature_1'] != corr['feature_2']]
    corr = corr.sort_values('corr_val', ascending=False).drop_duplicates(subset=['corr_val'])
    return corr.head(n)
...
def generate_insights(df, max_insights=6):
    ...
    if not corr_df.empty:
        ...
        f1       = str(r["feature_1"])
        f2       = str(r["feature_2"])
        corr_val = float(r["corr_val"])
        strength = "very strong" if corr_val > 0.8 else "moderate" if corr_val > 0.5 else "weak"
        tag      = "alert" if corr_val > 0.7 else "info"
        insights.append((tag, f"Strongest correlation: '{f1}' ↔ '{f2}' (|r|={corr_val:.2f}, {strength})."))
